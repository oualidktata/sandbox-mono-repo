...
export class UsersFacade {
  ...
  interests$ = this.interestService
    .getAll()
    .pipe(
      map((interests) =>
        interests.map(
          (interest) =>
            <IInterestViewModel>{ id: interest.id, name: interest.name }
        )
      )
    );
  ...
  filteredFromServer$ = this.serverSideCriteria$.pipe(
    distinctUntilChanged(),
    switchMap((criteria) => {
      return this.service.getUsersByCriteria(criteria);
      //.pipe(retry(3), startWith([]));
    }),
    tap((data) => console.log(`filteredFromServer.length${data.length}`))
  );

  filteredFromClient$ = combineLatest([
  ...
  showNotificationSection = this.uiService.showNotificationSection;
  ...
  applyClientFilterToCachedUsers = (
    ...
    filteredUsers = filteredUsers.slice(0, Number(clientFilter.topFilter));
    return filteredUsers;
  };
  updateFilter(filter: string) {
    const updatedFilter = { ...this._state.clientSideFilters, filter };
    this.saveClientState(updatedFilter);
  }
  updateTopFilter(topFilter: string) {
    const updatedFilter = { ...this._state.clientSideFilters, topFilter };
    this.saveClientState(updatedFilter);
  }
  ...
  selectUser(user: User) {
    ...
    this.interests$.subscribe(
      (interests) =>
        (user.interestNames = interests.filter((i) =>
          user.interests.includes(i.id)
        ))
    );
